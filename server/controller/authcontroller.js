...
authcontroller.verify = (req, res, next) => {
  ...
  // save query to a const= verifyUser
  // which is set to 'SELECT username FROM users where username = $1
...
authcontroller.checkPw = (req, res, next) => {
  ...
  const passedInName = res.locals.username; // passed in
  const passedInPass = res.locals.password; // passed in
